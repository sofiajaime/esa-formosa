#############################################
# Curso ESA
# Script clase 5: Condiciones de vida
# Septiembre 2021
#############################################
...
#1. Importamos las bases descargadas y las pegamos---------------
rm(list = ls())

eph_totalurb_ind <- read.csv2("bases/personas_tot urb_3T_2018.txt", header = TRUE, sep = ";")
eph_totalurb_hog <- read.csv2("bases/hogar_tot urb_3T_2018.txt", header = TRUE, sep = ";")

eph_totalurb_hog <- eph_totalurb_hog %>%
  select(!c(PONDERA, PONDIH, ANO4, ITF, IPCF, PROVINCIA, AGLOMERADO)) # Quito aquellas variables de la base de hogares que están repetidas en la de individuos

eph_totalurb <- eph_totalurb_ind %>%
  left_join(eph_totalurb_hog, by = c("CODUSU", "NRO_HOGAR")) # Uno las bases

rm(eph_totalurb_hog, eph_totalurb_ind) # Borro las bases individual y hogar


#2. Construcción del esquema de clases de Torrado (mismo algoritmo que en clase 4) ---------------------------

## Separación de dígitos CNO -------------------
eph_totalurb$cno12 <- ifelse(nchar(eph_totalurb$PP04D_COD) > 4, str_sub(eph_totalurb$PP04D_COD, 1, 2), str_sub(eph_totalurb$PP04D_COD, 1, 1))
eph_totalurb$cno5 <- ifelse(nchar(eph_totalurb$PP04D_COD) > 4, str_sub(eph_totalurb$PP04D_COD, 5, 5), str_sub(eph_totalurb$PP04D_COD, 4, 4))

eph_totalurb$cno12 <- as.numeric(eph_totalurb$cno12)
eph_totalurb$cno5 <- as.numeric(eph_totalurb$cno5)


## Recodificación sector de actividad y tamaño del establecimiento -------------------

eph_totalurb <- eph_totalurb %>%
  mutate(sector_act = car::recode(eph_totalurb$PP04A, "1=2; 2:3=1; else=NA"))
...
eph_totalurb <- eph_totalurb %>%
  mutate(go = case_when((cno12 >= 1 & cno12 <=4) | (cno12 == 7) ~ 1,
...
eph_totalurb$go_f <- factor(eph_totalurb$go, labels = c("Empresarios, directores de empresas y funcionarios públicos superiores",
...
eph_totalurb$cso_agg_factor <- factor(eph_totalurb$cso_agg, labels = c("DIREC", "PROF",
                                                                     "PPE", "TECN", "PPA",
                                                                     "EAV", "TEA", "OCAL",
                                                                     "ONCAL", "TMARG", "EDOM"))
table(eph_totalurb$cso_agg_factor)
...
eph_totalurb$clase6_factor <- factor(eph_totalurb$clase6, labels = c("Clase alta",
...
eph_totalurb <- eph_totalurb %>%
  mutate(clase3 = car::recode(eph_totalurb$clase6, "1=1; 2:3=2; 4:6=3"))

eph_totalurb$clase3_factor <- factor(eph_totalurb$clase3, labels = c("Clase alta",
                                                                   "Clase media", "Clase obrera"))

#3. Exploramos los datos por clase social a nivel individual ------------------

freq(eph_totalurb$clase6_factor, weights = eph_totalurb$PONDERA,
                   cumul = FALSE, report.nas = FALSE) # Frecuencias relativas de la variable clase


eph_totalurb %>%
  group_by(clase6_factor) %>%
  summarise(promedio = weighted.mean(P47T, w = PONDII)) # Ingresos totales individuales por clase social

#4. Exploramos condiciones de vida a nivel hogar --------------------------

#Selecciono a los/as jefe/as de hogar
eph_hog <- eph_totalurb %>%
  filter(CH03 == 1)

#Calculo el ingreso per cápita del hogar por clase social
eph_hog %>%
  group_by(clase6_factor) %>%
  summarise(promedio = weighted.mean(IPCF, w = PONDIH))

#Calculo el nivel de hacinamiento por clase social
eph_hog$hacina1 <- eph_hog$IX_TOT / eph_hog$II1
eph_hog$hacinamiento <- ifelse(eph_hog$hacina1 > 3, "Hacinamiento crítico", "No hacinado")

ctable(eph_hog$clase6_factor, eph_hog$hacinamiento, weights = eph_hog$PONDERA, useNA = "no")

#Calculo de cantidad de miembros por clase social
eph_hog %>%
  group_by(clase6_factor) %>%
  summarise(cantidad_miembros = weighted.mean(IX_TOT, w = PONDIH),
            cantidad_menores = weighted.mean(IX_MEN10, w = PONDIH))
